...
def train_model(saving_path, name, model, train_dataset, batch_size, optimizer, num_epochs, lr, N, sos_token_id, eos_token_id, pad_token_id, max_seq_len, tokenizer, device, wandb_log):
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % 10 == 0:
            torch.save(model.state_dict(), '%s/%s_%d.pt'%(saving_path, name, epoch))
            torch.save(model.state_dict(), '%s/%s.pt'%(saving_path, name))
            if epoch > 10:
                os.system ("rm %s/%s_%d.pt"%(saving_path, name, epoch - 10))
