...
from utils.utils_eval import load_transcript, windows, segment_data
...
def get_wordseqs_data(stories):
    ...
    for story in stories:
        ...
        new_time_index = word_seqs[story].tr_times[5 + config.TRIM : -config.TRIM] - (5 + config.TRIM)
...
def build_train_dict (args):
    ...
    # training stories
    stories = []
    with open(os.path.join(args.data_path, config.DATA_TRAIN_DIR, "sess_to_story.json"), "r") as f:
   ...
   # fMRI responses data
    ...
    n_chunks = len (rresp) // config.CHUNKLEN
    rest_chunk = len (rresp) % config.CHUNKLEN

    if rest_chunk > 1:
        rresp_chunked = np.vsplit(rresp[:-rest_chunk], n_chunks)
        rresp_chunked.append (rresp[-config.CHUNKLEN:])
    else:
    ...
    if rest_chunk > 1:
        ...
        rtext_data_chunked.append (rtext_data[-config.CHUNKLEN:])
    else:
    ...
    folder_path = os.path.join(args.data_path, "processed_data", "fMRI_data_train_split", args.subject)
    ...
    with open('data/%s/train.json'%args.subject, 'w') as output_file:
...
def build_test_data (args):
    ...
    experiments = glob(os.path.join(args.data_path, config.DATA_TEST_DIR, "test_stimulus/*"))
    experiments = [os.path.basename(x) for x in experiments]


    for experiment in experiments:
        tasks = glob(os.path.join(args.data_path, config.DATA_TEST_DIR, "test_response", args.subject, experiment, "*"))
        tasks = [os.path.basename(x).split ('.')[0] for x in tasks]


        for task in tasks:
          folder_path = os.path.join(args.data_path, "processed_data", "fMRI_data_test_split", args.subject, experiment, task)
          ...
          try:
            transcript_data = load_transcript(experiment, task.split('_')[0])
          except:
            continue

          # load test responses
          hf = h5py.File(os.path.join(args.data_path, config.DATA_TEST_DIR, "test_response", args.subject, experiment, task + ".hf5"), "r")
          ...
          with open('data/%s/test_%s_%s.json'%(args.subject, experiment, task), 'w') as output_file:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--rebuild", '-rb', type = str)
    parser.add_argument("--experiment", '-exp', type = str)
    parser.add_argument("--gpt", type = str, default = "perceived")
    parser.add_argument("--data_path", type = str, default = "data/semantic_decoding")
    ...
    if os.path.isdir("data/%s"%args.subject):
        shutil.rmtree("data/%s"%args.subject)
    os.mkdir("data/%s"%args.subject)
