...
from src.transformers_src.Metric import word_overlap_percentage, jaccard_similarity, LPTS
...
def inference(model, saving_file, tokenizer, vocab_len, test_dataset, sos_token_id, eos_token_id, pad_token_id, max_seq_len, device):
    ...
    for batch in test_dataset:
        ...
        src, trg_sentences, chunk_number = batch["bold_signal"], batch["text_output"], batch["chunk_number"]
        ...
        bleu_score, lpips_dist, overlap_score, jaccard_score = print_output_sentence(output_ids, trg, chunk_number, saving_file,
...
def print_output_sentence(output, trg_des, chunk_number, saving_file, tokenizer, pad_token_id, eos_token_id, nlp_lpips):
    ...
    with open(saving_file, 'a') as f:
        f.write(f'{chunk_number[0]} ;###; {output_sentence1} ;###; {desc_sentence}\n')
        #f.write(f'{chunk_number[0]} ;###; {output_sentence1} ;###; {desc_sentence}')
